...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 3:
        ...
        # Prioridad: SKELETON_IMAGE (archivo concreto) > SKELETON_FOLDER (carpeta)
        if SKELETON_IMAGE and Path(SKELETON_IMAGE).is_file():
            image_arg = SKELETON_IMAGE
        elif Path(SKELETON_FOLDER).is_dir():
            image_arg = SKELETON_FOLDER
        else:
            raise FileNotFoundError(
                f"No se encontro ni SKELETON_IMAGE ({SKELETON_IMAGE!r}) "
                f"ni SKELETON_FOLDER ({SKELETON_FOLDER!r}). "
                f"Edita la configuracion al inicio del archivo.")
