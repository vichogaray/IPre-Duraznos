...
imagen = imread(
    r"C:\Users\vgara\OneDrive\Desktop\IPre\MASKS\imgs_frame28_00000.png",
    as_gray=True
)

# binarización (ajusta el umbral si es necesario)
binary = imagen > 0.5
...
if __name__ == "__main__":
    ...
    # Usa la imagen original (en BGR) solo como "lienzo" para dibujar
    im0 = cv2.imread(r"C:\Users\vgara\Downloads\peach.png")
    if im0 is None:
        raise FileNotFoundError("No se pudo leer peach.png. Revisa la ruta.")

    # Usa el esqueleto ya calculado arriba (boolean) y pásalo a uint8 0/1
    im = skeleton.astype(np.uint8)

    rects = []
    polys = traceSkeleton(im, 0, 0, im.shape[1], im.shape[0], 10, 999, rects)

    for l in polys:
        c = (int(200*random.random()), int(200*random.random()), int(200*random.random()))
        for i in range(len(l)-1):
            cv2.line(im0, (l[i][0], l[i][1]), (l[i+1][0], l[i+1][1]), c, 1)

    cv2.imshow("skeleton tracing", im0)
    cv2.waitKey(0)
